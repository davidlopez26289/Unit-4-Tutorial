...
//Run the init() function whrn the page loads
window.onload = init;
var puzzleCells;
var cellBackground;

function init(){
   //insert the tittle for the firdt puzzle
   document.getElementById("puzzleTitle").innerHTML = "Puzzle 1";

   //insert the html code for the first puzzle
   document.getElementById("puzzle").innerHTML = drawPuzzle(puzzle1Hint, puzzle1Rating, puzzle1);

   //add event handleres for the puzzle buttons
...
function setupPuzzle(){
   //create a slector that will find every td in the table harjie grid
   puzzleCells = document.querySelectorAll("table#hanjieGrid td");


   //set the intial color of every cell in the table to gold
   for(var i = 0; i <puzzleCells.length; i++){
      puzzleCells[i].style.backgroundColor = "rgb(233,207,29)"
      //sest the cell backgeoud color in response to the mouse down event
   ...
   //Create an event listener highlight incorrect cells
   document.getElementById("peek").addEventListener('click',
      function(){
         ...
         //Removes the hints after .5 seconds
         setTimeout(
            function(){
